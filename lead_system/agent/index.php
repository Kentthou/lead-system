...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_lead'])) {
    ...
    header("Location: index.php?date={$selected_date}&mode={$mode}&page={$page}&updated=1");
    exit;
}
...
if ($mode === 'paged') {
    ...
    $stmt = $db->prepare("SELECT id, number, company_name, description, status, notes, lead_date, updated_at 
                          FROM leads 
                          WHERE agent_id = ? AND lead_date = ? 
                          ORDER BY id ASC 
                          LIMIT ? OFFSET ?");
    $stmt->bind_param('isii', $agent_id, $selected_date, $per_page, $offset);
} else {
    $stmt = $db->prepare("SELECT id, number, company_name, description, status, notes, lead_date, updated_at 
                          FROM leads 
                          WHERE agent_id = ? AND lead_date = ? 
...
<div class="card mb-3">
  <div class="card-body">
    ...
    <!-- Compact Date Filter with Prev/Today/Next -->
      ...
      <div class="col-md-4">
        ...
        <div class="input-group">
          <a class="btn btn-outline-secondary"
             href="?date=<?= esc(date('Y-m-d', strtotime($selected_date . ' -1 day'))) ?>&mode=<?= esc($mode) ?>">
            ← Prev
          </a>
          <input type="date" name="date" value="<?= esc($selected_date) ?>" class="form-control" onchange="this.form.submit()">
          <a class="btn btn-outline-secondary" href="?date=<?= esc(date('Y-m-d')) ?>&mode=<?= esc($mode) ?>">Today</a>
          <a class="btn btn-outline-secondary"
             href="?date=<?= esc(date('Y-m-d', strtotime($selected_date . ' +1 day'))) ?>&mode=<?= esc($mode) ?>">
            Next →
          </a>
    ...
    <div class="d-flex justify-content-between align-items-center mb-3">
      ...
      <a href="?date=<?= esc($selected_date) ?>&mode=<?= $mode === 'all' ? 'paged' : 'all' ?>" class="btn btn-sm btn-outline-dark">
        ...
        <tbody>
          <?php foreach ($leads as $l): ?>
            <?php
              ...
              // Prepare modal HTML for this lead and store it to output after the table
            ...
            <tr class="<?= $row_class ?>">
              ...
              <td>
                <button type="button" class="btn btn-sm btn-outline-primary" data-bs-toggle="modal" data-bs-target="#<?= esc($modal_id) ?>">
                  Edit
                </button>
              </td>
            </tr>
          ...
          <ul class="pagination justify-content-center">
            <?php for ($p = 1; $p <= $total_pages; $p++): ?>
              <li class="page-item <?= $p === $page ? 'active' : '' ?>">
                <a class="page-link" href="?date=<?= esc($selected_date) ?>&mode=paged&page=<?= $p ?>"><?= $p ?></a>
              </li>
            <?php endfor; ?>
          </ul>
        </nav>
